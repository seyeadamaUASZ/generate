...
import { AppFonc } from 'src/app/utilisateur/models/appFonc';
...
export class ChoixFoncComponent implements OnInit {
	...
	listFonctionnalite() {
		...
		// this.fonctionnaliteService.listFonctionnalite(this.donnee.idModule).subscribe(data => {
		// 	if (data.statut) {
		// 		this.fonctionnalite = data.data;
		// 		alert(JSON.stringify(this.appFonc));
		// 		console.log(this.appFonc);
		// 		console.log('------------------------------');
		// 		this.dataSource = new MatTableDataSource<Fonctionnalite>(data.data);
		// 		this.dataSource.paginator = this.paginator;
		// 		this.dataSource.sort = this.sort;
		// 		//this.dataSource2 = this.dataSource.data.concat.apply.apply(this.dataSource.data);
		// 		//alert(JSON.stringify(this.appFonc))
		// 	} else {
