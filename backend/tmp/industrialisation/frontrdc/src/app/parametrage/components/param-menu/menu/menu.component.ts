...
/********************************************************* */
  ...
  constructor(private readonly translate: TranslateService, private menuService: MenuService, private dialogRef: MatDialog,private router: Router, private route: ActivatedRoute) {
   }
  ...
  listMenus() {		
		this.menuService.listeMenu().subscribe(data => {			
			if (data.statut) {
				//this.user = data.data;								
  ...
  addAction(node:MenuFlatNode){
     this.menuService.changeactions(node.menu.actions);
