...
public interface AppFoncRepository extends JpaRepository<AppFonc, Long> {
   ...
   /* @Query("SELECT f FROM Fonctionnalite f INNER JOIN AppFonc a ON (f.fonId=a.fonctionnalite.fonId) WHERE a.application =:application ")
    public List<Fonctionnalite> findByFonctionnalite(@Param("application") Application application);*/
     ...
     public AppFonc FindAppliFonIsActive(@Param("idApp") Long idApp, @Param("idFon") Long idFon);
         ...
         @Query( value = "SELECT *FROM tr_app_fonc tr JOIN tp_fonctionnalite f ON (f.fon_id = tr.appli_fonc_fon_id) where f.fon_mod_id=:fonModId and tr.appli_fonc_app_id=:appId group By f.fon_id", nativeQuery = true)
