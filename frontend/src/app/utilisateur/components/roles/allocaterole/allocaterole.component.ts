...
export class AllocateroleComponent implements OnInit {
  ...
  masterToggle(actions) {
    ...
    this.isAllSelected(actions) ? this.selectionAction.clear() :
      actions.forEach(row => this.selectionAction.select(row));
      //this.addActionInPrivilege(actions);
      actions.forEach((row)=>{this.addActionInPrivilege(row)})
  }
