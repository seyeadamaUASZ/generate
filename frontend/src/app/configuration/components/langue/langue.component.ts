...
export class LangueComponent implements OnInit {
  ...
  loading = false;
  langues;
	displayedColumns: string[] = [ 'lngCode', 'lngLibelleComplet', 'lngLangue','action_s'];
	selectedColumns: string[] = [ 'lngCode', 'lngLibelleComplet', 'lngLangue','action_s'];

  displayedColumnsTraductions: string[] = [ 'reference', 'defaultLangue', 'selectedLangue','action_s'];
  selectedColumnsTraductions: string[] = [ 'reference', 'defaultLangue', 'selectedLangue','action_s'];
  saveCols(){
    localStorage.setItem("langueCols1",this.selectedColumns.toString());
  }

  saveColsTraduction(){
    localStorage.setItem("traductionCols1",this.selectedColumnsTraductions.toString());
  }
  ngOnInit() {
    let cols = localStorage.getItem("langueCols1");
    if(cols != undefined){
      this.selectedColumns = cols.split(",");
    }
    let colsTraduction = localStorage.getItem("traductionCols1");
    if(colsTraduction != undefined){
      this.selectedColumnsTraductions = colsTraduction.split(",");
    }
    this.listLangues();
  }
