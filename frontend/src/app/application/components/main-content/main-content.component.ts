...
export class MainContentComponent implements OnInit, AfterViewInit {
  ...
  displayedColumns: string[] = ['appNom', 'appNomEntreprise', 'appStatus', 'action_s'];
  selectedColumns: string[] = ['appNom', 'appNomEntreprise', 'appStatus', 'action_s'];
  ...
  selectedColumnsModule: string[] = ["modNom", "modDescription", "actionModule"];

  saveCols(){
    localStorage.setItem("applicationCols1",this.selectedColumns.toString());
  }

  saveColsModule(){
    localStorage.setItem("moduleCols1",this.selectedColumnsModule.toString());
  }
  ...
  ngOnInit() {
    let colsApps = localStorage.getItem("applicationCols1");
    if(colsApps != undefined){
      this.selectedColumns = colsApps.split(",");
    }

    let colsMod = localStorage.getItem("moduleCols1");
    if(colsMod != undefined){
      this.selectedColumnsModule = colsMod.split(",");
    }
