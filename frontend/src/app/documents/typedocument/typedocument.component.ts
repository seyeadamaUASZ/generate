...
export class TypedocumentComponent implements OnInit {
  ...
  displayedColumns: string[] = ['tydLibelle', 'tydDescription', 'action_s'];
  selectedColumns: string[] = ['tydLibelle', 'tydDescription', 'action_s'];

  saveCols(){
    localStorage.setItem("typedocumentCols1",this.selectedColumns.toString());
  }
  ...
  ngAfterViewInit() {
    let cols = localStorage.getItem("typedocumentCols1");
    if(cols != undefined){
